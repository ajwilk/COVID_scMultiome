...
p <- ggscatter(meta.fig1, "days_post_pos_test","Age",
...
saveit(format = "pdf", height= 3,width=3.5)
...
p <- DimPlot(covid_combined,group.by = "Severity.max.final", cols=severity.cols, reduction = "umap", raster = F) + labs(x = "UMAP1", y = "UMAP2") + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) 
...
meta.use <- covid_combined@meta.data[covid_combined$Array.type=="PBMC",]
meta.use$cell.type <- meta.use$manual.coarse.idents
meta.use$cell.type <- as.character(meta.use$cell.type)
meta.use$cell.type[meta.use$cell.type=="Developing neutrophil"] <- "Developing\nneutrophil"
meta.use$cell.type[meta.use$cell.type=="CD16 Monocyte"] <- "CD16 Mono"
meta.use$cell.type[meta.use$cell.type=="CD14 Monocyte"] <- "CD14 Mono"
meta.use$cell.type[meta.use$cell.type=="Eos/Mast Prog"] <- "Eos/Mast\nProg"

meta.use <- meta.use[!(meta.use$manual.coarse.idents=="Platelet" | meta.use$manual.coarse.idents=="Neutrophil"),]
propPlot(meta.use, ncol = 2) + theme(legend.position = "bottom", legend.direction = "vertical") + labs(y = "Proportion of PBMCs (%)")
...
meta.full <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/patient_metadata.csv"))
atac.embed <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scATAC/atac_UMAP_cell_metadata.csv?raw=true"))
...
atac.translate <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scATAC/atac.donor.translate.csv")) %>% mutate_all(as.character)
...
p <- DimPlot(covid_combined, group.by = "celltype.l2", reduction = "mapped.umap", label = T, repel = T, label.size = 7, pt.size = 0)  + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) + NoLegend() + scale_color_igv()
p$layers[[1]]$aes_params$alpha = .2
p
saveit(name = "a",path = wnn_fig_path)

p <- DimPlot(covid_combined, cols = severity.cols, group.by = "Severity.max.final", reduction = "mapped.umap", pt.size = 0)  + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) + NoLegend()
p$layers[[1]]$aes_params$alpha = .2
p
...
perturb.score <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/perturb.score.filter_quality.rds?raw=true"))
...
anno <- unique(row_annotation[row_annotation$Donor %in% rownames(perturb.filtered),])
anno <- anno[match(rownames(perturb.filtered),anno$Donor),]
...
rna.marker.list <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/rna.marker.list.filter_quality.rds?raw=true"))
...
Heatmap(t(abs(perturb.filtered)), 
        top_annotation = ta,
        left_annotation = la,
        col = colorRamp2(breaks = c(0,4,8), colors = c("black","purple","yellow")),
        name = "Perturbation\nscore",
...
#panel d
p1 <- DimPlot(alldc.seu,group.by = "Severity.max.final", cols=severity.cols, reduction = "umap") + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4))) + NoLegend()

p2 <- DimPlot(alldc.seu,group.by = "celltype.l2", label = T, repel = T, reduction = "umap", label.size = 10) + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4))) + NoLegend() + scale_color_igv() + theme(text = element_text(face = "bold")) + theme(axis.title.x = element_text(face = "plain"), axis.title.y = element_text(face = "plain"))

CombinePlots(list(p1,p2))
# saveit(name = "d", path = wnn_fig_path, height = 5, width = 10)
saveit(name = "d", height = 5, width = 10)

#panel e
alldc.seu$celltype.l2.factor <- factor(alldc.seu$celltype.l2, levels=rev(c("cDC1","cDC2","pDC","ASDC")))
DotPlot(alldc.seu, features = rev(c("SIGLEC6","AXL","IL3RA","TCF4","CD1C","CLEC9A")), group.by = "celltype.l2.factor", scale.max = 20, cols = "RdYlBu", dot.scale = 15, assay = "RNA") + labs(x=NULL,y=NULL) + RotatedAxis() + theme(axis.text.x = element_text(size = 15, face = "italic"), axis.text.y = element_text(size=15))
# saveit(format = "pdf", name = "e", path = wnn_fig_path, height = 5, width = 10)
saveit(format = "pdf", name = "e", height = 4, width = 8)
...
saveit(format = "pdf", height = 6, width = 12, name = "f")

covid_combined$cell.type <- covid_combined$celltype.l2_neut
propPlot(covid_combined@meta.data[covid_combined$Array.type=="PBMC",],select.idents = c("ASDC","cDC1","cDC2","pDC")) + labs(y = "Proportion of PBMCs (%)") + theme(text = element_text(size = 20))
saveit(format = "pdf", height = 3.25, width = 20)

saveit(format = "pdf", height = 5, width = 20)
```
...
cells.keep <- cells.keep[cells.keep %notin% colnames(covid_combined)[covid_combined$celltype.l2=="CD8 Proliferating"]]
...
#panel g
p1 <- DimPlot(allcd8.seu,group.by = "Severity.max.final", cols=severity.cols, reduction = "umap") + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4))) + NoLegend()

p2 <- DimPlot(allcd8.seu,group.by = "celltype.l2", label = T, repel = T, reduction = "umap", label.size = 10) + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4))) + NoLegend() + scale_color_igv() + theme(text = element_text(face = "bold")) + theme(axis.title.x = element_text(face = "plain"), axis.title.y = element_text(face = "plain"))
...
#panel h
allcd8.seu$celltype.l2.factor <- factor(allcd8.seu$celltype.l2, levels = rev(c("CD8 Naive","CD8 TCM","CD8 TEM","CD8 Proliferating")))
...
### scRNA-seq Monocyte figure ###
```{r panel 3a}
mono_fig_path = "~/Figures/mono"

covid_mono <- subset(covid_combined, idents = c("1", "4", "11", "16"))
covid_mono <- RunPCA(covid_mono, verbose = FALSE)
covid_mono <- RunUMAP(covid_mono, dims = 1:50, verbose = FALSE)
covid_mono <- FindNeighbors(covid_mono, dims = 1:50, verbose = FALSE)
covid_mono <- FindClusters(covid_mono, resolution = 1, verbose = FALSE)
covid_mono <- update.metadata(covid_mono)

p <- DimPlot(covid_mono,group.by = "Severity.max.final", cols=severity.cols, pt.size = 0, reduction = "umap") + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title = element_text(size = 20)) + theme(legend.position = c(.9,.8)) + NoLegend()
...
myFeaturePlot(covid_mono, features = c("CD14","FCGR3A"), ncol = 1, height = 6, width = 3)
```

```{r}
volcano.markers <- FindMarkers(covid_mono, ident.1 = "6-7", ident.2 = "0", group.by = "Severity.current.final", assay = "RNA", logfc.threshold = 0)

markers.final <- volcano.markers 
markers.final$gene <- rownames(markers.final)
markers.final <- markers.final %>% filter_quality()
EnhancedVolcano(markers.final, x = "avg_logFC", y = "p_val_adj", lab = rownames(markers.final), FCcutoff = 0.25, xlim = c(-2,3), labSize = 5, labFace = "italic", boxedLabels = T, drawConnectors = T, selectLab = c("IFI27","CLU","IFITM3","MX1","PLAC8","S100A9","CD163","CD63","IL1R2","C3AR1","HLA-DPA1","HLA-DRA","IL1B","CD4","IL8","CCL5"), ylim = c(0, 400), arrowheads = F, title = NULL, subtitle = NULL, caption = NULL)
saveit()
```


```{r panel 3c}
FeatureBoxPlot(covid_mono, features = c("IFI27","IFITM3","CD163", "ARG1", "PLAC8", "MPO", "HLA-DRA", "CD4"), ncol = 2) + theme(aspect.ratio = 1, text = element_text(size = 20), strip.text = element_text(size = 20)) + theme(legend.position = "bottom", legend.box = "vertical") + NoLegend()
saveit(height = 12, width = 6, name = "c", format = "pdf")
```


```{r panel 3d}
mdsc.set #can be found in Supplementary Table 12
covid_mono <- AddModuleScore(covid_mono, features = list(mdsc.set$gene), name = "mdsc")

sepsis.set  #can be found in Supplementary Table 12
covid_mono <- AddModuleScore(covid_mono, features = list(sepsis.set$gene), name = "sepsis")

covid_mono <- AddModuleScore(covid_mono, features = list(ISG.list$gene), name = "IFN")

hla.set <- read.csv("~/Downloads/hla.genes.csv")$gene
covid_mono <- AddModuleScore(covid_mono, features = list(hla.set), name = "HLA")

FeatureBoxPlot(covid_mono, features = c("sepsis1","mdsc1","IFN1","HLA1"), ncol = 2) + theme(text = element_text(size=25), aspect.ratio = 1) + NoLegend() + labs(y = "Mean module score")
saveit(name = "d", format = "pdf", width = 9, height = 4.5,path=mono_fig_path)

FeatureBoxPlot(covid_mono, features = c("sepsis1","mdsc1","IFN1","HLA1"), ncol = 4) + theme(text = element_text(size=25), aspect.ratio = 1, strip.text.x = element_text(size = 0)) + NoLegend() + labs(y = "Mean module score")
saveit(name = "d", format = "pdf", width = 9, height = 4.5,path=mono_fig_path)

FeatureBoxPlot(covid_mono, features = c("IFN1","HLA1"), ncol = 2) + theme(text = element_text(size=20), aspect.ratio = 1, strip.text.x = element_text(size = 0)) + NoLegend() + labs(y = "Mean module score")
saveit(name = "d", format = "pdf", width = 8, height = 4,path=mono_fig_path)
FeatureBoxPlot(covid_mono, features = c("sepsis1","mdsc1"), ncol = 2) + theme(text = element_text(size=20), aspect.ratio = 1, strip.text.x = element_text(size = 0)) + NoLegend() + labs(y = "Mean module score")
saveit(name = "d2", format = "pdf", width = 8, height = 4,path=mono_fig_path)
```

```{r monocyte maturity}
gset <- getGEO("GSE98090", GSEMatrix =TRUE, AnnotGPL=FALSE)
...
### REMEMBER, genes enriched in the samples that are denoted as "0" will have the positive LogFC ###
gsms <- "111000XXXXXXXXXXXX"
pullTopGenes <- function(gset,gsms, quantile = 0.97) {
  ...
  gs <- factor(sml)
  groups <- make.names(c("Mature mono","cMoP"))
  levels(gs) <- groups
  gset$group <- gs
  ...
  tT <- subset(tT, select=c("ID","adj.P.Val","P.Value","t","B","logFC","SPOT_ID","GB_ACC","GENE_SYMBOL","SEQUENCE"))
  
  cmop.genes <- unique(tT[tT$logFC>quantile(tT$logFC,0.97),"GENE_SYMBOL"])
  return(cmop.genes)
}

cmop_derived.genes <- pullTopGenes(gset,gsms = "111000XXXXXXXXXXXX")
cmop.genes <- pullTopGenes(gset,gsms = "111XXXXXX000XXXXXX")
premono.genes <- pullTopGenes(gset,gsms = "111XXX000XXXXXXXXX")
rgmp.genes <- pullTopGenes(gset,gsms = "111XXXXXXXXX000XXX")
cmp.genes <- pullTopGenes(gset,gsms = "111XXXXXXXXXXXX000")
mono.genes <- pullTopGenes(gset,gsms = "000111111111111111")

covid_mono <- AddModuleScore(covid_mono, features = list(cmop_derived.genes), name = "cmop_derived")
covid_mono <- AddModuleScore(covid_mono, features = list(cmop.genes), name = "cmop")
covid_mono <- AddModuleScore(covid_mono, features = list(premono.genes), name = "premono")
covid_mono <- AddModuleScore(covid_mono, features = list(rgmp.genes), name = "rgmp")
covid_mono <- AddModuleScore(covid_mono, features = list(cmp.genes), name = "cmp")
covid_mono <- AddModuleScore(covid_mono, features = list(mono.genes), name = "mono")
```


```{r panel 3e}
#if we want to include the Sepsis and MDSC signature
data.plot <- reshape2::melt(covid_mono@meta.data[,c("cmp1","rgmp1","cmop_derived1","cmop1","premono1","sepsis1","mdsc1","mono1","Severity.max.final")])

data.plot$variable <- mapvalues(data.plot$variable,
                                 from = c("cmp1","rgmp1","cmop_derived1","cmop1","premono1","sepsis1","mdsc1","mono1"),
                                 to = c("CMP","rGMP","cMoP_Mo","cMoP","Pre-monocyte","Sepsis","MDSC","Mature mono"))

data.plot$variable <- factor(data.plot$variable,levels = c("CMP","rGMP","cMoP","Pre-monocyte","cMoP_Mo","Mature mono","MDSC","Sepsis"))

ggplot(data.plot, aes_string(y = "value", x = "variable")) + labs(y = "Module score") + 
  theme_bw() + theme(panel.grid.minor = element_blank(), 
                     panel.grid.major = element_blank(), 
                     axis.ticks.x = element_blank(), 
                     axis.text = element_text(color = "black"), 
                     axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + 
  # geom_violin(position = "dodge", aes_string(fill = "Severity.max.final"), color = NA) +
  geom_boxplot(position = "dodge", aes_string(color = "Severity.max.final", fill = "Severity.max.final"), outlier.colour = NA, alpha = 0.5) + 
  ...
  labs(fill = "Peak severity\nscore", x = NULL) + 
  guides(fill = guide_legend(override.aes = list(shape=15)),color=F)
saveit(height = 4, width = 9, format="pdf")
```

```{r panel 3f}
cors.make <- cor(covid_mono@meta.data[,c("cmp1","rgmp1","cmop_derived1","cmop1","premono1","sepsis1","mdsc1")])
colnames(cors.make) <- mapvalues(colnames(cors.make), 
                                 from = c("cmp1","rgmp1","cmop_derived1","cmop1","premono1","sepsis1","mdsc1"),
                                 to = c("CMP","rGMP","cMoP_Mo","cMoP","Pre-monocyte","Sepsis","MDSC"))
rownames(cors.make) <- mapvalues(rownames(cors.make), 
                                 from = c("cmp1","rgmp1","cmop_derived1","cmop1","premono1","sepsis1","mdsc1"),
                                 to = c("CMP","rGMP","cMoP_Mo","cMoP","Pre-monocyte","Sepsis","MDSC"))
pdf("~/Downloads/p.pdf", height = 4, width = 5.5)
Heatmap(cors.make, name = "Pearson's\nr")
dev.off()
```
...
myFeaturePlot(covid_combined, features = c("C19orf59","IRAK3","ANXA3","RBM47","TLR5"), ncol = 5, height = 5, width = 10*5/3)

FeatureBoxPlot(covid_mono, features = c("C19orf59","IRAK3","ANXA3","RBM47","TLR5"),ncol = 5)
saveit(name = "c", format = "pdf", height = 6*.8, width = 18*.8)

FeatureBoxPlot(neut_alone, features = c("C19orf59","IRAK3","ANXA3","RBM47","TLR5"),ncol=5)
saveit(name = "d", format = "pdf", height = 6*.8, width = 18*.8)
```

### Monocyte Figure 2 (Figure 4) ###
```{r figure 4}
FeatureBoxPlot(covid_mono, features = c("IL1B", "IL6", "CCL2", "TNF"), ncol = 4) + theme(aspect.ratio = 1, text = element_text(size = 25), strip.text = element_text(size = 29))
saveit(height = 5, width = 16, name = "a", format = "pdf")

mono.sev.markers <- FindMarkers(covid_mono, ident.1 = "6-7", ident.2 = "0", group.by = "Severity.current.final", assay = "RNA")

#export to iRegulon
test <- mono.sev.markers %>% filter(avg_logFC>0.25 & p_val_adj < 0.05) %>% rownames_to_column(var = "gene") %>% filter_quality()
write.csv(test, file = "~/Downloads/mono.sev.iregulon.csv", row.names = T, quote = F)
test <- mono.sev.markers %>% filter(avg_logFC<(-0.25) & p_val_adj < 0.05) %>% rownames_to_column(var = "gene") %>% filter_quality()
write.csv(test, file = "~/Downloads/mono.healthy.iregulon.csv", row.names = T, quote = F)

#import iRegulon results
ireg <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/mono.ireg_new.csv"))
ireg$null <- "A"
ireg <- ireg %>% arrange(NES)
ireg$TF <- factor(ireg$TF, levels = ireg$TF)

ggplot(ireg, aes_string(x = "null", y = "TF")) + 
  geom_point(aes_string(size = "Targets", color = "NES")) + 
  theme_cowplot() + 
  ...
  rotate_x_text()
saveit(height = 2.3, width = 10, format="pdf")

DotPlot(covid_mono, features = c("NFKB1","NFKB2","REL","RELA","RELB"), group.by = "Severity.current.final", cols = "RdYlBu", dot.scale = 15) + 
  rotate_x_text() + 
  theme(axis.text.x = element_text(face = "italic",size=20),
        axis.text.y = element_text(size = 20),
        axis.title.y = element_text(size = 20)) + 
  labs(x = NULL, y = "Severity score at draw")
saveit(format = "pdf", height = 4.5, width = 6)
```
...
#panel A
pbmc.only.meta <- covid_combined@meta.data[covid_combined$Array.type=="PBMC",]
pbmc.only.meta$cell.type <- pbmc.only.meta$celltype.l2
covid.seuAbundances.meta(pbmc.only.meta, by = "cell.type", group_by = "Severity.current.final", 
                         each.pt = "Donor",
  ...
  theme(text = element_text(size = 20), aspect.ratio = 1, strip.text.x = element_text(size=0)) + NoLegend()
...
propPlot(as.data.frame(cell_metadata), each.pt = "Sample", group_by = "current_severity_bin", color_by = "peak_severity_bin", select.idents = "NK") + labs(y = NULL, x = NULL) + NoLegend() + theme(text = element_text(size = 20), aspect.ratio = 1, strip.text.x = element_text(size=0))
saveit(height = 3.5, width = 4.5, name = "a_ATAC", path = nk_fig_path, format = "pdf")
```


```{r panel 6c}
#panel B
DimPlot(covid_NK,group.by = "Severity.max.final", cols=severity.cols, reduction = "umap", pt.size = 0) + labs(x = "UMAP1", y = "UMAP2", title = NULL) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4))) + theme(aspect.ratio = 1) + NoLegend()
saveit(name = "b",path = nk_fig_path)

myFeaturePlot(covid_NK, features = c("FCGR3A","NCAM1","GZMB","PRF1","MKI67", "XAF1"), ncol = 2, order.cells = F, height = 5.25, width = 5.25)
```
...
```{r panel 8b}
#panel B
neut.markers <- build.deg.matrix(compartment = "Neut") #set path to location of neutrophil DEG lists available on the repo
...
latent_time <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/devneut_latent_time.csv"), row.names = 1)
...
public <- read_tsv("~/Downloads/GSE157103_genes.tpm.tsv")
...
classification <- read.xlsx(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/overmyer_fatal_case_classification.xlsx?raw=true"))
classification <- read.xlsx("/Volumes/GoogleDrive/My Drive/Blish Lab/00 - All Server Data and Folders/COVID_sc/scRNA/analyses/death_pred/fatal_SOFA.xlsx")

classification.sub <- classification[classification$fatal==1,]

test$fatal <- ifelse(rownames(test@meta.data) %in% classification.sub$record_id,1,0)
...
firstroc <- roc(test$fatal,test$test1,plot=T,legacy.axes=T,col="#b84f37",lwd=4,asp=NA,print.auc=TRUE, 
    print.auc.y=0.95, print.auc.x = 1,
    auc.polygon = TRUE, auc.polygon.col = "#b84f3722")
firstroc <- roc(classification$fatal,classification$SOFA,plot=T,legacy.axes=T,col="steelblue",lwd=4,asp=NA,print.auc=TRUE, print.auc.y = 0.4, auc.polygon = TRUE, auc.polygon.col = "#377eb822", add=T)
dev.off()
```
...
FeatureBoxPlot(HSPC.seu, features = c("CDK6", "SOX4", "CHD4", "HNRNPA2B1", "PTMA", "YBX1"), ncol = 6)
saveit(format = "pdf",height = 3.75, width = 3*6,name = "i")
```
...
```{r arraytype}
p <- DimPlot(covid_combined,group.by = "Array.type", cols=c("steelblue","red2"), reduction = "umap", pt.size = 0) + labs(x = "UMAP1", y = "UMAP2") + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), aspect.ratio = 1) + labs(title = NULL) 
p$layers[[1]]$aes_params$alpha = .3
p$data$cell.type <- covid_combined$manual.coarse.idents
p <- LabelClusters(p, id = "cell.type", size = 6)
p + guides(colour = guide_legend(override.aes = list(alpha = 1,size=4)))
saveit(name = "a",width = 7, height = 7)
...
rna.meta.upset <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/rna_meta.csv"))
atac.meta.upset <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scATAC/atac_meta.csv"))
atac.meta.upset <- atac.meta.upset[atac.meta.upset$record_id %in% atac.embed$Donor,]
cytof.meta.upset <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/cytof_meta.csv"))
meta.upset <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/patient_metadata.csv"))
...
```{r mapping freq}
mapping <- read_tsv(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/azimuth_pred.tsv?raw=true"))
...
```{r mait props}
cytof.mait <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/cytof_MAIT_props.csv"))
cytof.files <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/cytof_MAIT_meta.csv"))
...
ggplot(df, aes(x=freq,y=pctMAIT,color=Severity,shape=Acuity)) + 
  ...
  labs(y = "% MAIT CyTOF", x = "% MAIT scRNA") + guides(color = guide_legend(override.aes = aes(label = ""))) + NoLegend()
# saveit(format = "pdf", name = "d",path=supp_2_path, height = 5, width = 7)
saveit(format = "pdf", name = "d", height = 3.5, width = 3.5)
```
...
meta.use <- covid_combined@meta.data[covid_combined$Array.type=="PBMC",]
meta.use$nkcell <- ifelse(rownames(meta.use) %in% real.nk.cells, T, F)

fq <- as.data.frame(prop.table(table(meta.use$nkcell, meta.use$Donor), 2) *100)
fq <- fq[fq$Var1==T,]
fq <- fq[!is.na(fq$Freq),]

cytof <- read.csv(url("https://github.com/ajwilk/COVID_scMultiome/raw/main/data/scRNA/donor_NK_proportions_CyTOF.csv"))
colnames(cytof) <- c("cell.type","Donor","cytof")
colnames(fq) <- c("cell.type","Donor","rna")

cytof <- cytof[cytof$Donor %in% fq$Donor,]

merged <- merge(cytof,fq,by = "Donor")
ei <- unique(meta.use[,c("acuity","Severity.max.final","Donor")])
merged.clin <- merge(merged,ei,by = "Donor")

ggplot(merged.clin, aes(x=rna,y=cytof,color=Severity.max.final,shape=acuity)) + 
  geom_point(size=3) + scale_color_manual(values = severity.cols) +
  geom_smooth(method='lm',aes(group=1),color="grey40") + 
  stat_regline_equation(mapping = aes(group=1)) + 
  stat_cor(mapping = aes(group=1), label.y = 60) + 
  theme_bw() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(),
        strip.background = element_rect(fill = NA, color = NA), 
        strip.text = element_text(face = "bold"), axis.ticks.x = element_blank(), 
        ...
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        text = element_text(size = 20), aspect.ratio = 1) + 
  labs(y = "% NK CyTOF", x = "% NK scRNA", color = "Severity", shape = "Acuity") + 
  guides(color = guide_legend(override.aes = aes(label = ""))) + NoLegend()
saveit(format = "pdf", height = 3.5, width = 3.5) 
saveit(format = "png", height = 5, width = 7) 
```


```{r mono comp}
cytof #instantiate this object from AWS bucket

results <- list()
for (i in 1:nrow(cytof)){
  results[[i]] <- lm(gene~severity, data = data.frame(gene=cytof@assays$protein@data[i,],severity=as.numeric(as.character(cytof$Severity.current))))$coefficients[2]
}
names(results) <- rownames(cytof)

covid_mono <- AddModuleScore(covid_mono, features = list(c("ULBP1","ULBP2")),name = "ULBP_score")
covid_mono <- AddModuleScore(covid_mono, features = list(c("MICA","MICB")),name = "MIC_score")
covid_mono <- AddModuleScore(covid_mono, features = list(c("TNFRSF10A","TNFRSF10B")),name="DR_score")
covid_mono <- AddModuleScore(covid_mono, features = list(grep("^HLA-DR",rownames(covid_mono),value=T)),name="HLADR_score")


rna.mono.data <- data.frame("CD16"=covid_mono@assays$RNA@data["FCGR3A",],
                            # "CD112"=covid_mono@assays$RNA@data["NECTIN2",],
                            "CD11b"=covid_mono@assays$RNA@data["ITGAM",],
                            "CD4"=covid_mono@assays$RNA@data["CD4",],
                            "HLA-C"=covid_mono@assays$RNA@data["HLA-C",],
                            "HLA-E"=covid_mono@assays$RNA@data["HLA-E",],
                            "CD95"=covid_mono@assays$RNA@data["FAS",],
                            # "CD111"=covid_mono@assays$RNA@data["NECTIN1",],
                            "CD155"=covid_mono@assays$RNA@data["PVR",],
                            "CD7"=covid_mono@assays$RNA@data["CD7",],
                            "CD48"=covid_mono@assays$RNA@data["CD48",],
                            "CD54"=covid_mono@assays$RNA@data["ICAM1",],
                            "CD161"=covid_mono@assays$RNA@data["KLRB1",],
                            "LLT-1"=covid_mono@assays$RNA@data["CLEC2D",],
                            "CD15"=covid_mono@assays$RNA@data["FUT4",],
                            "CCR2"=covid_mono@assays$RNA@data["CCR2",],
                            "DR4-5"=covid_mono$DR_score1,
                            "CD14"=covid_mono@assays$RNA@data["CD14",],
                            "HLA-DR"=covid_mono$HLADR_score1,
                            "CD58"=covid_mono@assays$RNA@data["CD58",],
                            "CD33"=covid_mono@assays$RNA@data["CD33",],
                            "CD56"=covid_mono@assays$RNA@data["NCAM1",],
                            "B7-H6"=covid_mono@assays$RNA@data["NCR3LG1",],
                            "ULBP-1-2-5-6"=covid_mono$ULBP_score1,
                            "MICA-B"=covid_mono$MIC_score1)

rna.lm <- list()
for (i in 1:ncol(rna.mono.data)){
  rna.lm[[i]] <- lm(gene~severity, data = data.frame(gene=rna.mono.data[,i],severity=as.numeric(as.character(covid_mono$current_severity))))$coefficients[2]
}
names(rna.lm) <- colnames(rna.mono.data)

rna.lm.df <- as.data.frame(t(as.data.frame(rna.lm))) %>% rownames_to_column(var="gene")
colnames(rna.lm.df) <- c("gene","rna_coef")

cytof.lm.df <- as.data.frame(t(as.data.frame(results))) %>% rownames_to_column(var="gene")
colnames(cytof.lm.df) <- c("gene","cytof_coef")

coef.merge <- merge(rna.lm.df,cytof.lm.df,by="gene")


ggscatter(coef.merge, x = "rna_coef", y = "cytof_coef",
          add = "reg.line",                         # Add regression line
          conf.int = TRUE,
          label = "gene"
          ) + stat_cor(label.x = -0.075, label.y = 0.2) + labs(y = "CyTOF severity beta coefficient", x = "RNA severity beta coefficient") + scale_y_continuous(expand = expansion(mult = c(0.15, 0.15))) + scale_x_continuous(expand = expansion(mult = c(0.15, 0.15))) #+ theme(text=element_text(size=15))

saveit(height = 3.5, width = 3.5, format = "pdf")
```

```{r nk comp}
cytof.nk #instantiate this object from AWS bucket

results <- list()
for (i in 1:nrow(cytof.nk)){
  results[[i]] <- lm(gene~severity, data = data.frame(gene=cytof.nk@assays$protein@data[i,],severity=as.numeric(as.character(cytof.nk$Severity.current))))$coefficients[2]
}
names(results) <- rownames(cytof.nk)

covid_nk <- subset(covid_combined, cells = colnames(covid_combined)[grepl("NK",covid_combined$celltype.l2)])
covid_nk <- AddModuleScore(covid_nk, features = list(c("ULBP1","ULBP2")),name = "ULBP_score")
covid_nk <- AddModuleScore(covid_nk, features = list(c("MICA","MICB")),name = "MIC_score")
covid_nk <- AddModuleScore(covid_nk, features = list(c("TNFRSF10A","TNFRSF10B")),name="DR_score")
covid_nk <- AddModuleScore(covid_nk, features = list(grep("^HLA-DR",rownames(covid_nk),value=T)),name="HLADR_score",nbin=10)


rna.nk.data <- data.frame("CD16"=covid_nk@assays$RNA@data["FCGR3A",],
                            "NKp30"=covid_nk@assays$RNA@data["NCR3",],
                            "Siglec7"=covid_nk@assays$RNA@data["SIGLEC7",],
                            "NKG2C"=covid_nk@assays$RNA@data["KLRC2",],
                            "KIR2DL1"=covid_nk@assays$RNA@data["KIR2DL1",],
                            "CD94"=covid_nk@assays$RNA@data["KLRD1",],
                            # "KIR2DL5"=covid_nk@assays$RNA@data["KIR2DL5",],
                            "CD62L"=covid_nk@assays$RNA@data["SELL",],
                            # "KIR2DS4"=covid_nk@assays$RNA@data["KIR2DS4",],
                            "NKp46"=covid_nk@assays$RNA@data["NCR1",],
                            "NKG2D"=covid_nk@assays$RNA@data["KLRK1",],
                            "TIGIT"=covid_nk@assays$RNA@data["TIGIT",],
                            "x2B4"=covid_nk@assays$RNA@data["CD244",],
                            "FAS-L"=covid_nk@assays$RNA@data["FASLG",],
                            "HLA-DR"=covid_nk$HLADR_score1,
                            "CD96"=covid_nk@assays$RNA@data["CD96",],
                            "KIR2DL3"=covid_nk@assays$RNA@data["KIR2DL3",],
                            "CD69"=covid_nk@assays$RNA@data["CD69",],
                            "CD2"=covid_nk@assays$RNA@data["CD2",],
                            "LILRB1"=covid_nk@assays$RNA@data["LILRB1",],
                            "FcRg"=covid_nk@assays$RNA@data["FCER1G",],
                            "Syk"=covid_nk@assays$RNA@data["SYK",],
                            "CD38"=covid_nk@assays$RNA@data["CD38",],
                            "CD8"=covid_nk@assays$RNA@data["CD8A",],
                            "Ki-67"=covid_nk@assays$RNA@data["MKI67",],
                            # "KIR2DS2"=covid_nk@assays$RNA@data["KIR2DS2",],
                            "DNAM-1"=covid_nk@assays$RNA@data["CD226",],
                            "PD1"=covid_nk@assays$RNA@data["PDCD1",],
                            "NKG2A"=covid_nk@assays$RNA@data["KLRC1",],
                            "NTB-A"=covid_nk@assays$RNA@data["SLAMF6",],
                            "KIR3DL1"=covid_nk@assays$RNA@data["KIR3DL1",],
                            "CD56"=covid_nk@assays$RNA@data["NCAM1",],
                            "Perforin"=covid_nk@assays$RNA@data["PRF1",])

rna.lm <- list()
for (i in 1:ncol(rna.nk.data)){
  rna.lm[[i]] <- lm(gene~severity, data = data.frame(gene=rna.nk.data[,i],severity=as.numeric(as.character(covid_nk$current_severity))))$coefficients[2]
}
names(rna.lm) <- colnames(rna.nk.data)
...
ggscatter(coef.merge, x = "rna_coef", y = "cytof_coef",
          ...
          label = "gene",
          repel = T
          ) + stat_cor(label.x = -0.025, label.y = .15) + labs(y = "CyTOF severity beta coefficient", x = "RNA severity beta coefficient") + scale_y_continuous(expand = expansion(mult = c(0.15, 0.15))) + scale_x_continuous(expand = expansion(mult = c(0.15, 0.2))) 
...
```{r age corr}
meta.use <- covid_combined@meta.data
meta.use$cell.type <- as.character(mapvalues(meta.use$manual.coarse.idents, 
                                from = c("Developing neutrophil",
                                         "CD16 Monocyte",
                                         "CD14 Monocyte",
                                         "Eos/Mast Prog"),
                                to = c("Developing\nneutrophil",
...
meta.use <- meta.use[!(meta.use$manual.coarse.idents=="Platelet" | meta.use$manual.coarse.idents=="Neutrophil"),]
meta.use <- meta.use[!(meta.use$Array.type=="WB"),]

fq <- as.data.frame(prop.table(table(meta.use$cell.type, meta.use$Donor), 2) *100)
colnames(fq) <- c("cell.type","Donor","Freq")
ei <- unique(meta.use[,c("Donor","Severity.max.final","Severity.current.final", "acuity","Age")])
df <- merge(fq,ei,by = "Donor")

ggplot(df, aes_string(x="Age",y="Freq",color="Severity.max.final",shape="acuity")) + 
  ...
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(),
        ...
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        text = element_text(size = 20), aspect.ratio = 1) + 
  labs(y = "Proportion of PBMCs (%)", x = "Age", color = "Severity", shape = "Acuity") + 
  guides(color = guide_legend(override.aes = aes(label = ""))) + 
  scale_x_continuous(limits = c(min(df$Age),max(df$Age)))
saveit(height = 10, width = 10, format="pdf")
```

```{r age regress}
test <- df %>% group_by(cell.type) %>%
  group_modify(~ cbind(.x, residuals = lm(Freq~Age,.x)$residuals, fitted.values = lm(Freq~Age,.x)$fitted.values))
y_var = "residuals"
ggplot(test, aes_string(y = y_var, x = "Severity.current.final")) + 
  labs(x = "WHO severity score at draw", y = "Proportion of PBMCs (%)") + 
  theme_bw() + theme(panel.grid.minor = element_blank(), 
                     panel.grid.major = element_blank(), 
                     strip.background = element_rect(fill = NA, color = NA), 
                     strip.text = element_text(face = "bold"),
  ...
  guides(fill = FALSE) + geom_boxplot(aes_string(x = "Severity.current.final"), alpha = 0.25, outlier.color = NA) +
  geom_point(size = 4, position = position_jitter(width = 0.25),
             aes_string(x = "Severity.current.final", y = y_var, color = "Severity.max.final", shape = "acuity")) +
  theme(panel.grid.major = element_line(color = "grey", size = 0.25), aspect.ratio = 1,
        text = element_text(size = 20)) +
  scale_color_manual(values = severity.cols) +
  scale_fill_manual(values = severity.cols) + 
  scale_y_continuous(expand = expand_scale(mult = c(0.05, 0.1))) + 
  ggpubr::stat_compare_means(mapping = aes_string(y_var), comparisons = my_comparisons.current, label = "p.signif", 
                             p.adjust.methods = "bonferroni",
                             method = "wilcox.test") + 
  labs(color = "Peak severity\nscore", shape = "Acuity") + theme(legend.position = "right", legend.direction = "vertical")
saveit(height = 10, width = 10, name = y_var, format = "pdf")
```
...
ISG.matrix <- ifelse(ISG.markers>0,1,0)[,2:ncol(ISG.markers)]
...
cytokine.list <- c(grep("^CSF", rownames(covid_combined), value = T),
                   grep("^IFN", rownames(covid_combined), value = T),
                   grep("^IL", rownames(covid_combined), value = T),
                   grep("^TNFSF", rownames(covid_combined), value = T),
                   grep("^CXCL", rownames(covid_combined), value = T),
                   grep("^CCL", rownames(covid_combined), value = T),
...
cytokine.matrix <- ifelse(cytokine.markers>0,1,0)[,2:ncol(cytokine.markers)]
...
```{r pDC IRF7}
pdc.seu <- subset(covid_combined, cells = colnames(covid_combined)[covid_combined$celltype.l2=="pDC"])
pdc.cluster.seu <- subset(covid_combined, cells = colnames(covid_combined)[covid_combined$manual.coarse.idents=="pDC"])
FeatureBoxPlot(pdc.seu, features = "IRF7") + theme(strip.text.x = element_text(size=25))
FeatureBoxPlot(pdc.cluster.seu, features = "IRF7") + theme(strip.text.x = element_text(size=25))

saveit(format = "pdf", height = 4, width = 6)
```
...
```{r comp perturb}
mild.perturb <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/perturb.score.filter_quality_convalescent_mild.rds?raw=true"))
moderate.perturb <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/perturb.score.filter_quality_convalescent_moderate.rds?raw=true"))

mild.perturb.m <- Reduce(cbind,mild.perturb )
colnames(mild.perturb.m) <- names( mild.perturb  )
mild.anno <- unique(row_annotation[row_annotation$Donor %in% rownames(mild.perturb.m),])
mild.anno <- mild.anno[match(rownames(mild.perturb.m),mild.anno$Donor),]
mild.anno$perturb <- rowSums(abs(mild.perturb.m))
moderate.perturb.m <- Reduce(cbind,moderate.perturb )
colnames(moderate.perturb.m) <- names( moderate.perturb  )
moderate.anno <- unique(row_annotation[row_annotation$Donor %in% rownames(moderate.perturb.m),])
moderate.anno <- moderate.anno[match(rownames(moderate.perturb.m),moderate.anno$Donor),]
moderate.anno$perturb <- rowSums(abs(moderate.perturb.m))

perturb.sum <- rbind(mild.anno,moderate.anno)

ggplot(perturb.sum, aes_string(y = "perturb", x = "Severity.final.max")) + 
  labs(x = NULL, y = "Proportion of PBMCs (%)") + 
  theme_bw() + theme(panel.grid.minor = element_blank(), 
                     panel.grid.major = element_blank(), 
                     strip.background = element_rect(fill = NA, color = NA), 
                     strip.text = element_text(face = "bold"),
                     ...
                     axis.text = element_text(color = "black"), 
                     axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  guides(fill = FALSE) + geom_boxplot(aes_string(x = "Severity.final.max"), alpha = 0.25, outlier.color = NA) +
  geom_point(size = 4, position = position_jitter(width = 0.25),
             aes_string(x = "Severity.final.max", y = "perturb", color = "Severity.final.max")) +
  theme(panel.grid.major = element_line(color = "grey", size = 0.25), aspect.ratio = 1,
        text = element_text(size = 20)) +
  scale_color_manual(values = severity.cols[2:3]) +
  scale_fill_manual(values = severity.cols[2:3]) + 
  scale_y_continuous(expand = expand_scale(mult = c(0.05, 0.1))) + 
  ggpubr::stat_compare_means(mapping = aes_string(y_var), comparisons = list(c("1-3","4-5")), label = "p.signif", 
                             p.adjust.methods = "bonferroni",
                             method = "wilcox.test") + 
  labs(x = "Peak severity score", color = NULL, y = "Cumulative\nperturbation score") + NoLegend()
saveit(height = 4, width = 4, format = "pdf")
```


```{r mild perturb}
rna.marker.list_mild <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/rna.marker.list_convalescent_mild.rds?raw=true"))
...
perturb.score <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/perturb.score.filter_quality_convalescent_mild.rds?raw=true"))
perturb.score.m <- Reduce(cbind,perturb.score )
colnames(perturb.score.m) <- names( perturb.score  )
rownames(perturb.score.m) <- mapvalues(rownames(perturb.score.m), from = as.character(rna.meta$Donor), to = as.character(rna.meta$Donor.full))
...
```{r perturb moderate}
rna.marker.list_moderate <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/rna.marker.list_convalescent_moderate.rds?raw=true"))
...
perturb.score <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/perturb.score.filter_quality_convalescent_moderate.rds?raw=true"))
perturb.score.m <- Reduce(cbind,perturb.score )
colnames(perturb.score.m) <- names( perturb.score  )
rownames(perturb.score.m) <- mapvalues(rownames(perturb.score.m), from = as.character(rna.meta$Donor), to = as.character(rna.meta$Donor.full))
...
Figure S3a
```{r UMAP}
neut_combined <- readRDS(url("https://covid-scmultiome-public.s3-us-west-1.amazonaws.com/201130--neut_combined.seu.rds")) 
...
meta.use$cell.type <- as.character(mapvalues(meta.use$manual.coarse.idents, 
                                ...
                                to = c("Developing\nneutrophil",
                                       ...
                                       "CD14 Mono",
                                       "Eos/Mast\nProg")))
meta.use$fatal <- factor(ifelse(meta.use$Severity.max.final=="8","Fatal","Non-fatal"), levels = c("Non-fatal","Fatal"))

fq <- as.data.frame(prop.table(table(meta.use$cell.type, meta.use$Donor), 2) *100)
colnames(fq) <- c("cell.type","Donor","Freq")
ei <- unique(meta.use[,c("Donor","Severity.max.final","Severity.current.final", "acuity","Age","fatal")])
df <- merge(fq,ei,by = "Donor")
df <- df[df$cell.type=="Developing\nneutrophil",]
df <- df[df$Severity.current.final=="6-7",]

ggplot(df, aes_string(y = "Freq", x = "fatal")) + 
  labs(x = NULL, y = "Proportion of PBMCs (%)") + 
  theme_bw() + theme(panel.grid.minor = element_blank(), 
                     panel.grid.major = element_blank(), 
                     strip.background = element_rect(fill = NA, color = NA), 
                     ...
                     axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
  facet_wrap("cell.type", scales = "free_y", ncol = 3) + 
  guides(fill = FALSE) + geom_boxplot(aes_string(x = "fatal"), alpha = 0.25, outlier.color = NA) +
  geom_point(size = 4, position = position_jitter(width = 0.25),
             aes_string(x = "fatal", y = "Freq", color = "Severity.max.final")) +
  theme(panel.grid.major = element_line(color = "grey", size = 0.25), aspect.ratio = 1,
        text = element_text(size = 20)) +
  scale_color_manual(values = severity.cols[4:5]) +
  scale_fill_manual(values = severity.cols[4:5]) + 
  scale_y_continuous(expand = expand_scale(mult = c(0.05, 0.1))) + 
  ggpubr::stat_compare_means(mapping = aes_string(y_var), comparisons = list(c("Non-fatal","Fatal")), label = "p.signif", 
                             p.adjust.methods = "bonferroni",
                             method = "wilcox.test") + 
  labs(color = "Peak severity\nscore")
saveit(height = 4, width = 5, format = "pdf")
```

### Supplementary Tables ###

```{r}
st_path = "path_to_write_supp_tables"
STlookup #lookup table for each mention of SuppTable in text
```
...
# meta.st$Age_orig <- NULL

meta.full <- read.csv("~/Downloads/patient_metadata.csv")
priormeds <- read.csv("~/Desktop/COVID_sc_metadata/priormeds_PPID_for_angela.csv")
meta.st <- merge(meta.full, priormeds[,c("Donor","ibuprofen_prior","abx_prior","remdesivir_prior","systemic_steroids_prior__excludes_inhaled")], by = "Donor", all.x = T)
cols.recode <- c("Donor","ibuprofen_prior","abx_prior","remdesivir_prior","systemic_steroids_prior__excludes_inhaled")
for (i in cols.recode) {
  meta.st[,i] <- mapvalues(meta.st[,i], from = c("X","",NA), to = c("Yes","No","No"))
}
...
```{r RNAManualDEG}
name = "RNAManualDEG"
# rna.markers <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/manual.coarse.idents_markers.rds?raw=true"))
rna.markers <- readRDS("~/Downloads/manual.coarse.idents_markers.rds")
...
```{r RNASeuratDEG}
name = "RNASeuratDEG"
# rna.markers <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/celltype.l2_markers.rds?raw=true"))
rna.markers <- readRDS("~/Downloads/210428--rna.markers.celltypel2.rds")
my.write(rna.markers, 
         file = paste0(st_path,"SupplementaryTable_",
                       STlookup[STlookup$name==name,"mention"],
                       ".csv"),
...
```{r RNASeuratDEP}
name = "RNASeuratDEP"
# imputed.protein.markers <- readRDS(url("https://github.com/ajwilk/COVID_scMultiome/blob/main/data/scRNA/imputed_protein_markers.rds?raw=true")) %>% rename(imputed.protein = gene)
protein.markers <- readRDS("~/Downloads/210428--protein.markers.celltypel2.rds") %>% rename(imputed.protein = gene)
my.write(protein.markers, 
         file = paste0(st_path,"SupplementaryTable_",
                       STlookup[STlookup$name==name,"mention"],
                       ".csv"),
         header = paste0("Supplementary Table ",
...
RNAModules <- makePaddedDataFrame(list(ISG = unique(as.character(ISG.list$gene)),
                   ...
                   sepsis_monocytes = unique(as.character(sepsis.set$gene)),
                   ...
                   cmop_derived = cmop_derived.genes,
                   cmop = cmop.genes,
                   premono = premono.genes,
                   rgmp = rgmp.genes,
                   cmp = cmp.genes,
                   mono = mono.genes)) # the last 6 are from mono.projection.Rmd

RNAModules <- as.data.frame(apply(RNAModules,2,FUN = function(x) {ifelse(is.na(x),"",x)}))
...
```{r RNAMonoSevDEG}
name = "RNAMonoSevDEG"
Idents(covid_mono) <- "Severity.current.final"
RNAMonoSevDEG.markers <- FindAllMarkers(covid_mono, assay = "RNA", only.pos = T) %>% filter_quality()
Idents(covid_mono) <- "seurat_clusters"
...
my.write(RNAMonoSevDEG.markers, 
         ...
         header = paste0("Supplementary Table ",
                         STlookup[STlookup$name==name,"mention"],
                         ". DEGs for Monocytes by severity. scRNA-seq DEGs for each severity class were calculated using FindAllMarkers, where cells belonging to each severity class are compared to all other cells. Cells manually-annotated as monocytes were used as input. Only DEGs with a >0.25 log(fold-change) are shown, and cell quality-associated genes are removed. Statistical significance is determined by Seurat's implementation of the Wilcoxon rank-sum test, and two-sided p values are provided."),
         row.names = F)
```
...
mono_sev_ireg <- read.csv("~/Downloads/mono.sev.iregulon.csv")
mono_healthy_ireg <- read.csv("~/Downloads/mono.healthy.iregulon.csv")
# devneut_ireg #can be found in Supplementary Table 
# neut_sev_ireg #can be found in Supplementary Table 
devneut_ireg <- read.csv("~/Downloads/devneut.ireg.csv")
neut_sev_ireg <- read.csv("~/Downloads/neut.sev.ireg.csv")

iRegulonList <- makePaddedDataFrame(list(Neutrophils = neut_sev_ireg$gene,
                                         DevNeuts = devneut_ireg$gene,
                                         Mono_healthy = mono_healthy_ireg$gene,
                                         Mono_severe = mono_sev_ireg$gene))
...
```{r RNAArrayTypeProps}
name = "RNAArrayTypeProps"
fq <- prop.table(table(covid_combined$manual.coarse.idents, covid_combined$orig.ident), 2) *100
arrayprops <- reshape2::melt(fq, value.name = "freq", varnames = c("cell.type", "orig.ident"))
ei = unique(covid_combined@meta.data[,c("orig.ident","Array.type")])
arrayprops <- merge(arrayprops,ei,by = "orig.ident")
...
my.write(arrayprops, 
         ...
         header = paste0("Supplementary Table ",
                         STlookup[STlookup$name==name,"mention"],
                         ". Proportions of cell types in RNA dataset per donor by sample type (ACK-lysed whole blood [WB] or PBMC)."),
         row.names = F)
```

```{r ATACProps}
name = "ATACProps"
atacprops.meta <- atac.embed
atacprops.meta$cell_type <- mapvalues(atacprops.meta$cell_type, from = c("B naive","B memory","B immature"), to = c("B intermediate","B naive","B memory"))
fq <- prop.table(table(atacprops.meta$cell_type, atacprops.meta$Donor), 2) *100
atacprops <- reshape2::melt(fq, value.name = "freq", varnames = c("cell.type", "Donor"))
ei = unique(atacprops.meta[,c("Donor","current_severity_bin.confirm","peak_severity_bin")])
atacprops <- merge(atacprops,ei,by = "Donor")
colnames(atacprops) <- c("Donor","cell.type","freq","current_severity","peak_severity")
my.write(atacprops, 
         file = paste0(st_path,"SupplementaryTable_",
                       STlookup[STlookup$name==name,"mention"],
                       ".csv"),
         header = paste0("Supplementary Table ",
                         STlookup[STlookup$name==name,"mention"],
                         ". Proportions of cell types in scATAC dataset per donor."),
         row.names = F)
```

```{r RNASeuratProps}
name = "RNASeuratProps"
fq <- prop.table(table(covid_combined$celltype.l2_neut, covid_combined$Donor), 2) *100
l2props <- reshape2::melt(fq, value.name = "freq", varnames = c("cell.type", "Donor"))
ei = unique(covid_combined@meta.data[,c("Donor","Severity.max.final","Severity.current.final")])
l2props <- merge(l2props,ei,by = "Donor")
...
my.write(l2props, 
         ...
         header = paste0("Supplementary Table ",
                         STlookup[STlookup$name==name,"mention"],
                         ". Proportions of Seurat v4-annotated cell types per donor in scRNA-seq dataset."),
         row.names = F)
```
